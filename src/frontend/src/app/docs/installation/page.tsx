  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      ...
      <DocSection title="Khởi chạy nhanh với Docker Compose (Khuyến nghị)" id="docker">
        <p>
          Axiom DX-OS được container hóa toàn bộ bằng Docker Compose, bao gồm Next.js Frontend,
          FastAPI Backend, LiveKit Media Server, PostgreSQL 16 và Redis:
        </p>
        <pre>
          <code>{`# 1. Clone mã nguồn dự án
...
docker compose ps`}</code>
        ...
        <Alert type="success" title="Cổng dịch vụ mặc định">
          ...
          <br />• <strong>Tài khoản mặc định:</strong> <code>admin@axiom.com</code> /{' '}
      ...
      <DocSection title="Môi trường Cục bộ (Local Development)" id="local">
        ...
        <pre>
          <code>{`# Backend (Python 3.12+ và uv):
uv sync --all-extras
...
npm run dev`}</code>
        ...
        <Alert type="warning" title="Mô hình AI Cục bộ (Ollama)">
          Đảm bảo Ollama đang chạy tại máy host (mặc định <code>http://localhost:11434</code>) với
          model Qwen để phục vụ trích xuất nhiệm vụ và tóm tắt biên bản cuộc họp.
        </Alert>
      </DocSection>
    </div>
  );
