                ...
                >
                  ...
                  <div className="p-3 bg-slate-50 flex-1 flex flex-col min-h-0">
                    ...
                    {isEditingAgenda ? (
                      ...
                      <div className="flex-1 overflow-y-auto pr-1 text-xs bg-white rounded-lg scrollbar-thin mt-2">
                          ...
                          {user?.role !== 'MEMBER' &&
                            topics.some((t) => t.status !== 'COMPLETED') && (
                              <button
                                ...
                                className="px-2 py-1 bg-blue-100 text-blue-700 rounded-md text-[10px] font-bold hover:bg-blue-200 transition-colors shadow-sm cursor-pointer"
                              >
                        ...
                        {topics.length === 0 ? (
                          ...
                          <div className="flex flex-col gap-2">
                            {topics.map((t) => (
                              <div
                                key={t.id}
                                className={`p-2.5 rounded-lg border transition-all ${t.status === 'IN_PROGRESS' ? 'border-blue-400 bg-blue-50 shadow-sm' : 'border-slate-200 bg-white'}`}
                              >
                                <div
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    <span
                                      className={`text-[10px] uppercase font-bold tracking-wider ${t.status === 'COMPLETED' ? 'text-slate-400' : t.status === 'IN_PROGRESS' ? 'text-blue-600' : 'text-slate-400'}`}
                                    >
                                      [
                                      {t.status === 'COMPLETED'
                                        ? 'Đã xong'
                                    ...
                                    <span
                                      className={`font-semibold text-[11px] ${t.status === 'COMPLETED' ? 'line-through text-slate-400' : t.status === 'IN_PROGRESS' ? 'text-slate-900' : 'text-slate-600'}`}
                                    >
                                      {t.title}
                                    </span>
                                  </div>
                                ...
                                {expandedTopicId === t.id && (
                                  <div className="mt-2 pt-2 border-t border-slate-200/60">
                                    ...
                                    {decisions.filter((d) => d.topic_id === t.id).length === 0 ? (
                                      <div className="text-[10px] italic text-slate-400 ml-2 mb-2">
                                        Chưa có quyết định.
                                      </div>
                                    ) : (
                                      <div className="mb-2 flex flex-col gap-1.5">
                                        {decisions
                                          .filter((d) => d.topic_id === t.id)
                                          .map((d) => (
                                            <div
                                              key={d.id}
                                    ...
                                    <div className="text-[10px] font-bold text-slate-500 uppercase mt-2 mb-1 tracking-wider">
                                      Công việc (Tasks)
                                    </div>
                                    {actionItems.filter((task) => task.topic_id === t.id).length ===
                                    0 ? (
                                      <div className="text-[10px] italic text-slate-400 ml-2">
                                        Chưa có task.
                                      </div>
                                    ) : (
                                      <div className="flex flex-col gap-1.5">
                                        {actionItems
                                          .filter((task) => task.topic_id === t.id)
                                          .map((task) => (
                                            <div
                                              key={task.id}
