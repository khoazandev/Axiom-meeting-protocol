          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {meetings.map((meeting) => (
                <Card
                  key={meeting.id}
                  className="border-border/50 shadow-sm rounded-xl overflow-hidden hover:border-primary/50 transition-colors group"
                >
                  <CardHeader className="p-6 pb-4">
                    <div className="flex items-start justify-between">
                      <CardTitle className="text-xl font-medium tracking-tight line-clamp-1">
                        {meeting.title}
                      </CardTitle>
                      {meeting.is_active && (
                  ...
                  <CardContent className="p-6 pt-0 space-y-6">
                    ...
                    <Link href={`/meetings/${meeting.id}`} className="block">
                      <Button
                        className="w-full rounded-lg font-medium group-hover:bg-primary group-hover:text-primary-foreground transition-colors"
                        variant="secondary"
                      >
