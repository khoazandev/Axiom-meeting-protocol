        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">
              Lịch Trình Họp & Standup Của Tôi
            </h2>
            <span className="text-[11px] font-extrabold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
              TUẦN 37 (07/09 - 11/09/2026)
            </span>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Tổng hợp các ca họp Sprint, Daily Standup 15 phút và lịch 1-on-1 với Trưởng phòng.
          </p>
        </div>

        {/* Anti-burnout Status */}
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-xs text-amber-800 dark:text-amber-300 font-bold">
          <AlertTriangle size={14} className="text-amber-600 shrink-0" />
          <span>Thời lượng họp tuần: 17.5h (Đang bật chế độ Deep Work)</span>
        </div>
      </div>
          ...
          >
            <div className="flex items-start sm:items-center gap-4">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span
                    className={`text-[10px] font-extrabold px-2 py-0.5 rounded-md border ${getTypeBadge(
                      ev.type
                    )}`}
                  >
                    {ev.type}
