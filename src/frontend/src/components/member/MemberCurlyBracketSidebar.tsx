...
export function MemberCurlyBracketSidebar({
  activeSection,
  onSelectSection,
  onOpenProfile,
  onLogout,
}: MemberCurlyBracketSidebarProps) {
  const { user } = useAuthStore();
  // Visibility & Interaction States
  const [isOpen, setIsOpen] = useState(false);
  const [isPinned, setIsPinned] = useState(false);
  const [hoveredSection, setHoveredSection] = useState<MemberSectionKey | null>(null);

  // Debounce ref to prevent accidental closing
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.metaKey || e.ctrlKey) {
        ...
        if (num >= 1 && num <= 6) {
          e.preventDefault();
          onSelectSection(MEMBER_NAV_SECTIONS[num - 1].id);
        ...
        <div className="relative rounded-2xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-[0_20px_50px_rgba(0,0,0,0.14),0_1px_3px_rgba(0,0,0,0.05),inset_0_1px_1px_rgba(255,255,255,0.6)] dark:shadow-[0_24px_54px_rgba(0,0,0,0.55),inset_0_1px_1px_rgba(255,255,255,0.08)] p-2 flex flex-col items-center gap-1.5">
          ...
          <div className="w-full flex flex-col items-center pt-1 pb-1.5 px-1">
            <div
              ...
              title="Axiom DX-OS Member Hub: Trực tuyến"
            >
          ...
          <nav className="flex flex-col items-center gap-1.5" aria-label="Member Navigation">
            {MEMBER_NAV_SECTIONS.map((section, idx) => {
          ...
          {/* Footer Controls: Member Profile Avatar + Pin Dock Button + Logout */}
          <div className="w-full flex flex-col items-center gap-1.5 pt-1.5 pb-1">
            ...
            {/* Member Profile Avatar (Click to open UserProfileModal) */}
            <button
              type="button"
              onClick={onOpenProfile}
              className="relative w-8 h-8 rounded-full overflow-hidden border border-emerald-400/80 ring-2 ring-emerald-100 dark:ring-emerald-950/60 hover:ring-emerald-500 transition-all cursor-pointer group"
...
export default MemberCurlyBracketSidebar;
