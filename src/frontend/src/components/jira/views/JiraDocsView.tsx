        ...
        <div className="flex items-center justify-between">
          ...
          {project.meeting_id && (
            <Link
              ...
              className="px-3 py-1.5 rounded-xl bg-primary/20 hover:bg-primary/30 text-primary border border-blue-500/30 text-xs font-semibold flex items-center gap-1.5 transition-all shadow-xs"
            >
      ...
      {meeting && (
        <div className="p-5 rounded-2xl bg-card border border-border space-y-4 shadow-xs">
          <div className="flex items-center justify-between border-b border-border/60 pb-3">
            ...
            <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-success/15 text-success border border-emerald-500/30">
              {meeting.status}
            </span>
          </div>
          ...
          <div className="space-y-3">
            ...
            {transcripts.length === 0 ? (
              <div className="p-6 text-center text-xs text-muted-foreground italic bg-card/30 rounded-xl border border-border/50">
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto custom-scrollbar pr-2">
                {transcripts.map((t) => (
                  <div
                    key={t.id}
                    className="p-3 rounded-xl bg-card/40 border border-border/60 text-xs space-y-1"
                  >
                    <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                      <span className="font-bold text-primary">{t.speaker_name}</span>
                      <span>
