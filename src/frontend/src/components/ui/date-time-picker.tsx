...
export function CustomDateTimePicker({
  ...
  const [hours, setHours] = useState(
    value ? initialDate.getHours().toString().padStart(2, '0') : '12'
  );
  ...
  if (value) {
    try {
      const d = new Date(value);
      displayText = d.toLocaleString('vi-VN', {
        hour: '2-digit',
        minute: '2-digit',
        day: '2-digit',
        month: '2-digit',
  ...
  return (
    <div className="relative flex-1" ref={containerRef}>
      ...
      {isOpen && (
        <div
          className={`absolute top-full ${align === 'left' ? 'left-0' : 'right-0'} mt-1 w-[280px] bg-card border border-border rounded-xl shadow-2xl z-50 p-3 flex flex-col gap-3 animate-in fade-in zoom-in-95 duration-100`}
        >
          {/* Header */}
